...
def test_positions_found_with_lower_notes_on_higher_strings() -> None:
    chord = music.Chord([music.Note("A", 2), music.Note("C#", 3)])
    expected = [
        {"E": 9, "A": 0},
        {"E": 5, "A": 4},
    ]
...
@pytest.mark.parametrize("frets,expected", [(10, {"E": 8, "A": 3}), (5, {"A": 3})])
...
def test_different_guitar_tunings(strings: list[tuple[str, int]], capo: int) -> None:
    guitar = music.Guitar(
        tuning={i: music.Note(*string) for i, string in enumerate(strings)}, capo=capo
    )
...
def test_guitar_extremes() -> None:
    guitar = music.Guitar(tuning={"E": music.Note("E", 2), "A": music.Note("A", 2)}, frets=3)
...
def test_print_more_complex() -> None:
    ...
    guitar = music.Guitar(
        tuning={string: music.Note.from_string(note) for string, note in open_d.items()}
    )
...
def test_parse_tuning(string: str) -> None:
    expected = {"E": music.Note("E", 2), "A": music.Note("A", 2)}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Cmaj7",
            {"chord_note": "C", "root": "C", "quality": "maj7", "notes": ["C", "E", "G", "B"]},
        ),
        ("C7", {"chord_note": "C", "root": "C", "quality": "7", "notes": ["C", "E", "G", "Bb"]}),
        ("Cm7", {"chord_note": "C", "root": "C", "quality": "m7", "notes": ["C", "Eb", "G", "Bb"]}),
        (
            "Cm7b5",
            {"chord_note": "C", "root": "C", "quality": "m7b5", "notes": ["C", "Eb", "Gb", "Bb"]},
        ),
        (
            "Cdim7",
            {"chord_note": "C", "root": "C", "quality": "dim7", "notes": ["C", "Eb", "Gb", "A"]},
        ),
        (
            "Caug7",
            {"chord_note": "C", "root": "C", "quality": "aug7", "notes": ["C", "E", "Ab", "Bb"]},
        ),
        ...
        (
            "F#m7b5",
            {"chord_note": "F#", "root": "F#", "quality": "m7b5", "notes": ["F#", "A", "C", "E"]},
        ),
        # inversions
        (
            "Bbmaj7/D",
            {"chord_note": "Bb", "root": "D", "quality": "maj7", "notes": ["D", "F", "A", "Bb"]},
        ),
        (
            "F#m7b5/E",
            {"chord_note": "F#", "root": "E", "quality": "m7b5", "notes": ["E", "F#", "A", "C"]},
        ),
...
def test_get_chord_with_add_octave(
    raise_octave: dict[str, int], expected: list[music.Note]
) -> None:
...
def test_get_all_chords() -> None:
    actual = music.ChordName("C").get_all_chords(lower=music.Note("C", 0), upper=music.Note("E", 2))
    expected = [
...
def test_get_all_chords_with_repeats() -> None:
    actual = music.ChordName("C").get_all_chords(
        lower=music.Note("C", 0),
        upper=music.Note("E", 1),
        allow_repeats=True,
...
def test_thumb_position_not_barre() -> None:
    position = music.GuitarPosition({"E": 3, "A": 5, "D": 3, "G": 4, "B": 6})
...
def test_get_all_chords_allow_identical() -> None:
    ...
    actual = c.get_all_chords(
        upper=music.Note("G", 1), max_notes=4, allow_repeats=True, allow_identical=True
    )
    assert set(actual) == set(expected)
...
def test_get_all_chords_extension_again() -> None:
    ...
    actual = c.get_all_chords(
        upper=music.Note("C", 2), max_notes=6, allow_repeats=True, allow_identical=False
    )
    assert set(actual) == set(expected)
...
@pytest.mark.parametrize("name,frequency", [("A4", 440.0), ("A3", 220.0), ("C4", 261.626)])
...
@pytest.mark.parametrize("note,line", [("C4", 0), ("C5", 7), ("E4", 2), ("Eb4", 2), ("E#4", 2)])
...
def test_chord_comparison() -> None:
    ...
    assert music.Chord([music.Note("C", 0), music.Note("D", 1)]) < music.Chord(
        [music.Note("C", 0), music.Note("E", 1)]
    )
...
def test_semitone_distance() -> None:
    c1 = music.Chord(
        [music.Note("C", 3), music.Note("Eb", 3), music.Note("F", 3), music.Note("A", 3)]
    )
    c2 = music.Chord(
        [
            music.Note("C", 3),
...
def test_voice_leading() -> None:
    cp = music.ChordProgression([music.ChordName(n) for n in ["Em7", "A7", "Dm7", "G7", "CM7"]])
    result1 = cp.optimal_voice_leading(
        lower=music.Note("C", 2), upper=music.Note("C", 4), use_dijkstra=True
    )
    result2 = cp.optimal_voice_leading(
        lower=music.Note("C", 2), upper=music.Note("C", 4), use_dijkstra=False
    )
