...
@pytest.mark.parametrize("name", ["Gmaj7", "C#m7b11/E", "D"])
@pytest.mark.parametrize("args", [[], ["-n 3"], ["-f 5"], ["-r"], ["-i"], ["-p"], ["-g"]])
...
@pytest.mark.parametrize("notes", ["G1,B1,D2,F2", "G1,B3,D2,F#2"])
@pytest.mark.parametrize("args", [[], ["-n 3"], ["-f 5"], ["-r"], ["-i"], ["-p"], ["-g"]])
...
def test_voice_leading(args: list[str]) -> None:
    result = subprocess.run(
        ["python", os.path.join(PROJ_DIR, "main.py"), "voice_leading", *args], capture_output=True
    )
    assert result.returncode == 0
