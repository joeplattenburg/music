...
def guitar_positions():
    if request.method == "POST":
        ...
        tuning = (
            "standard" if guitar.tuning_name == "standard" else "custom;" + request.form["tuning"]
        )
...
@app.route(
    "/guitar_positions/notes/<notes_string>" "/<top_n>/<max_fret_span>/<tuning>/<allow_thumb>"
)
def guitar_positions_display_notes(
    notes_string: str, top_n: str, max_fret_span: str, tuning: str, allow_thumb: str
) -> str:
    ...
    chord.to_audio(sample_rate=SAMPLE_RATE, duration=NOTE_DURATION).write_wav(
    ...
    guitar = (
        music.Guitar()
        if tuning_ == "standard"
        else music.Guitar(tuning=music.Guitar.parse_tuning(tuning_.split(";")[1]))
    )
    t1 = time.time()
    ...
    return render_template(
        ...
        chord=chord,
        tuning=tuning_,
        positions=positions_printable,
        chords_n=1,
        total_n=positions_all,
        playable_n=len(positions_playable),
...
def guitar_positions_display_name(
    ...
    low_chord.to_audio(sample_rate=SAMPLE_RATE, duration=NOTE_DURATION).write_wav(
        os.path.join(PROJ_DIR, "static", "temp.wav")
    )
    ...
    return render_template(
        ...
        tuning=tuning_,
        positions=positions_printable,
        chords_n=len(chords_playable),
        total_n=len(positions_all),
        playable_n=len(positions_playable),
        elapsed_time=elapsed_time,
    )
...
def voice_leading_display(chords_string: str, lower: str, upper: str):
    ...
    audio = reduce(
        add,
        (chord.to_audio(sample_rate=SAMPLE_RATE, duration=NOTE_DURATION) for chord in opt_chords),
    )
    ...
    elapsed_time = f"{(time.time() - t1):.2f}"
    return render_template(
        "voice_leading_display.html", chords=chords_string, elapsed_time=elapsed_time
    )
