...
def guitar_positions(args: argparse.Namespace):
    guitar = instruments.Guitar(tuning=args.tuning, tuning_name=args.tuning_name, capo=args.capo, frets=args.frets)
    engine = engines.FretboardEngine(max_fret_span=args.max_fret_span)
    if args.notes:
        note_list = [primitives.Note.from_string(note) for note in args.notes.split(',')]
        chord = primitives.Chord(note_list)
        print(f'You input the chord: {chord}')
        positions_playable = engine.chord_to_guitar_positions(
            chord=chord, guitar=guitar, include_unplayable=False,
        )
    elif args.name:
        print(f'You input the chord: {args.name}')
        chord_name = primitives.ChordName(args.name)
        positions_all = engine.chord_name_to_all_guitar_positions(
    ...
    positions = instruments.GuitarPosition.sorted(positions_playable)[:args.top_n]
...
def guitar_optimal_progression(args: argparse.Namespace):
    ...
    cp = primitives.ChordProgression([primitives.ChordName(n) for n in args.chords])
    guitar = instruments.Guitar(tuning_name=args.tuning_name, tuning=args.tuning)
    engine = engines.FretboardEngine()
    result = engine.chord_progression_to_optimal_guitar_positions(
        chord_progression=cp, guitar=guitar, allow_repeats=args.allow_repeats, respect_fingers=args.fingers
    )
...
def voice_leading(args: argparse.Namespace):
    ...
    result = cp.optimal_voice_leading(
        lower=primitives.Note.from_string(args.lower),
        upper=primitives.Note.from_string(args.upper),
    )
...
def main() -> None:
    ...
    guitar_positions_parser.add_argument(
        '--max-fret-span', '-f', type=int, default=instruments.DEFAULT_MAX_FRET_SPAN,
    ...
    guitar_positions_parser.add_argument(
        ...
        help='Name of alternate tuning', choices=list(instruments.Guitar.TUNINGS.keys())
    )
    guitar_positions_parser.add_argument(
        '--tuning', type=instruments.Guitar.parse_tuning, default=None,
        help=(
    ...
    guitar_positions_parser.add_argument(
        '--frets', type=int, default=instruments.Guitar.DEFAULT_FRETS,
